...
class IntegrationRegistry:
    ...
    def list(self):
        ...
        for connector_id in ids:
            ...
            out.append({'id':connector_id,'name':name,'provider':manifest.provider if manifest else connector_id,'configured':configured,'capabilities':capabilities,'operations':operations,'healthy':healthy,'state':state.get('state'),'granted_scopes':state.get('granted_scopes',[]),'last_success_at':state.get('last_success_at'),'last_checked_at':state.get('last_checked_at'),'last_error':state.get('last_error_message'),'last_error_code':state.get('last_error_code'),'revocation_status':state.get('revocation_status','none'),'auth_type':manifest.authentication_type if manifest else None,'required_scopes':list(manifest.required_oauth_scopes) if manifest else [],'optional_scopes':list(manifest.optional_oauth_scopes) if manifest else []})
        return out
