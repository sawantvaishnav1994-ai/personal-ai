...
def register(reg):
    ...
    def prep(action, tool_name, *, requires_approval, upload=False, explicit_submit=False):
        def prepare(parameters):
            ...
            if upload:
                params['approved_roots'] = approved_roots(params, 'read')
    ...
    def hardened_result(action, parameters):
        result = transactional.execute(action, parameters)
    ...
    def add(name, description, action, risk, policy_operation, *, target_type='domain', reauth=False, upload=False, verification=True, explicit_submit=False):
        ...
        reg.register(Tool(
            ...
            minimum_risk=risk,
            prepare=prep(action, name, requires_approval=requires_approval, upload=upload, explicit_submit=explicit_submit),
            on_reject=transactional.cancel,
