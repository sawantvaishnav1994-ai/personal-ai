...
def build_observation_record(*, binding, transaction_id: str, application: dict, screen: dict, browser: dict | None, reason: str, initiator: str) -> dict:
    ...
    required = ('observation_id', 'owner_id', 'device_id', 'session_id', 'transaction_id', 'screenshot_evidence_ref', 'screen_fingerprint')
    if any(not record.get(key) for key in required): raise ObservationSafetyError('verification_failed', 'observation record is incomplete')
...
def observation_context(record: dict) -> dict:
    return {key: record.get(key) for key in ('application_identity','process_identity','window_identity','browser_context_identity','browser_tab_identity','browser_origin','normalized_url','sanitized_dom_digest','accessibility_tree_digest','actionable_element_digest','frame_origins_digest')}
...
def verify_material_context(*, expected: dict, current: dict, target_binding: dict, current_browser: dict | None, data_root: Path, now: float | None = None):
    ...
    if expected.get('browser_tab_identity') or expected.get('browser_origin'):
        ...
        if int((current.get('sensitivity') or {}).get('sensitive_region_count') or 0)>int((expected.get('sensitivity') or {}).get('sensitive_region_count') or 0): raise ObservationSafetyError('target_changed', 'a new password or secret field appeared before dispatch')
