...
def install(cls):
    """Install subordinate orchestration adapters on the canonical AdvancedAutonomy class."""
    ...
    def validate_executable(self,plan_id,*,owner_id='owner'):
        ...
        if plan['state'] in {'CANCELLED','COMPLETED','FAILED','UNCERTAIN','PAUSED','BLOCKED'}:
    ...
    def execute_task(self,plan_id,task_id,*,owner_id='owner',device_id=None,session_id=None,reauthenticated_at=None,background=False):
        ...
        if self.operations is None:
            return self.mark_task(plan_id,task_id,'COMPLETED',owner_id=owner_id,result_ref='read-only-orchestration',verified=True)
        ...
        op_plan=self.operations.create_plan(
            f"P10:{plan['goal_id']}:{task_id}",
            [{'id':task_id,'instruction':task.get('objective',''),'requested_tool':requested_tool,'parameters':dict(task.get('parameters') or {}),'sensitivity':task.get('privacy','internal')}],
            owner_id=owner_id,
            budget_policy={'max_steps':1,'max_tool_calls':1,'max_model_calls':1,'max_retries':0},
        )
        ...
        if isinstance(result,dict) and result.get('approval_required'):
            task['status']='WAITING_APPROVAL'; plan['state']='WAITING_APPROVAL'
        elif operation and operation.get('status')=='waiting_approval':
            task['status']='WAITING_APPROVAL'; task['operation_id']=operation.get('operation_id'); task['approval_ref']=operation.get('approval_id'); plan['state']='WAITING_APPROVAL'
        elif operation and operation.get('outcome_state') in {'VERIFIED','RECOVERED'}:
            task['status']='COMPLETED'; task['result_ref']=operation.get('operation_id'); plan['state']='COMPLETED' if all(x['status']=='COMPLETED' for x in plan['tasks']) else 'RUNNING'
        elif operation and operation.get('outcome_state')=='UNCERTAIN':
            task['status']='UNCERTAIN'; task['operation_id']=operation.get('operation_id'); plan['state']='UNCERTAIN'
        elif operation and operation.get('status') in {'failed','cancelled'}:
            task['status']='FAILED' if operation.get('status')=='failed' else 'CANCELLED'; plan['state']='FAILED' if task['status']=='FAILED' else 'CANCELLED'
        self._save_plan(plan); self._event('task_dispatched',goal_id=plan['goal_id'],plan_id=plan_id,task_id=task_id,operation_id=task.get('operation_id'),state=task['status'])
        return plan
    ...
    def approve_task(self,plan_id,task_id,*,owner_id='owner',device_id=None,session_id=None,reauthenticated_at=None):
        ...
        plan['state']='COMPLETED' if all(x['status']=='COMPLETED' for x in plan['tasks']) else ('FAILED' if task['status']=='FAILED' else 'RUNNING')
    ...
    def deny_task(self,plan_id,task_id,*,owner_id='owner',device_id=None,session_id=None):
        ...
        self.operations.reject(task['operation_id'],owner_id=owner_id,device_id=device_id,session_id=session_id)
        task['status']='CANCELLED'; plan['state']='CANCELLED'; return self._save_plan(plan)
    ...
    def create_background_job(self,plan_id,task_id,*,owner_id='owner'):
        ...
        self._db.execute('DELETE FROM p10_jobs WHERE id NOT IN (SELECT id FROM p10_jobs ORDER BY updated_at DESC LIMIT 500)'); self._db.commit(); return job

    def job(self,job_id,*,owner_id='owner'):
        if self._db is None: raise KeyError('job store not configured')
        row=self._db.execute('SELECT owner_id,document FROM p10_jobs WHERE id=?',(job_id,)).fetchone()
        if not row or row['owner_id']!=owner_id: raise KeyError('job not found')
        return json.loads(row['document'])

    cls.validate_executable=validate_executable
    cls.execute_task=execute_task
    cls.approve_task=approve_task
    cls.deny_task=deny_task
    cls.cancel_governed=cancel_governed
    cls.create_background_job=create_background_job
    cls.job=job
    cls._sanitize_event=sanitize
