...
def main():
 ...
 with tempfile.TemporaryDirectory() as d:
  root=Path(d);db=root/'soak.sqlite3';store=MemoryStore(db);vault=SecretVault(root/'vault.json','soak-password');gateway=DeviceGateway(Registry(),Events());realtime=OpenAIRealtimeVoiceSession(realtime_settings(),Events())
  while time.time()<deadline:
   ...
   if i%500==0:
    vault.rotate_password(f'soak-password-{i}');assert vault.get('rotation-probe')==str(i)
  ...
  vault_reloaded=SecretVault(root/'vault.json',f'soak-password-{((i-1)//500)*500}' if i>500 else 'soak-password');assert vault_reloaded.get('rotation-probe') is not None
