...
def main():
    ...
    router=GovernedModelRouter(settings()); iterations=0; local_ok=0; fallback_ok=0; privacy_blocks=0; timeouts=0; restarts=0
    ...
    print({'p9_iterations':iterations,'rss_start':start,'rss_peak':peak,'rss_end':end,'rss_growth':end-start,'local_ok':local_ok,'fallback_ok':fallback_ok,'privacy_blocks':privacy_blocks,'timeouts':timeouts,'restarts':restarts,'history_size':len(router.observability.generations)})
