...
def _record():
    ...
    return {'redaction_status':'sanitized','visual_evidence_unavailable':False,'coordinate_space_version':1,'expires_at':time.time()+60,'owner_id':'owner','device_id':'device','session_id':'session','_sanitized_png':raw,'sanitized_checksum':hashlib.sha256(raw).hexdigest()}
...
def test_guard_rejects_unavailable_or_unknown_provenance():
    ...
    unknown = _record(); unknown['coordinate_space_version'] = 999
