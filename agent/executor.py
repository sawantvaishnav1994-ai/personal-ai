...
class AgentExecutor:
    def __init__(self,*,models,tools,memory,events,second_brain=None,approval_ttl_seconds:int=300,telemetry=None):
        self.models=models;self.tools=tools;self.memory=memory;self.events=events;self.second_brain=second_brain;self.planner=Planner(models,tools);self.approvals=ApprovalManager(approval_ttl_seconds);self._paused={};self._lock=threading.RLock();self.telemetry=telemetry
    def _observe(self,name,start):
        if self.telemetry:self.telemetry.observe(name,(time.perf_counter()-start)*1000)
    def chat(self,text,*,confirmed_tools:set[str]|None=None):
        turn_start=time.perf_counter()
        try:return self._chat(text,confirmed_tools=confirmed_tools)
    ...
    def _chat(self,text,*,confirmed_tools=None):
        ...
        try:plan=self.planner.plan(text,context=context);self._observe('agent.plan_ms',start)
            ...
            self._observe('agent.plan_ms',start);start=time.perf_counter();answer=self.models.chat(text,history=history[:-1]);self._observe('model.chat_ms',start);self.memory.add_message('assistant',answer);self.events.emit('state',state='speaking');return answer
    ...
    def _continue(self,execution_id,text,plan,index,results,history):
        ...
        while index<len(steps):
            ...
            if not decision.allowed:
                ...
                audit={'approval_id':ticket.id,'execution_id':execution_id,'tool':tool.name,'parameter_hash':ticket.parameter_hash,'expires_at':ticket.expires_at};self.memory.audit('approval','required',audit);self.events.emit('approval.required',**audit);raise ConfirmationRequired(tool.name,params,step.get('description',''),approval_id=ticket.id,execution_id=execution_id,expires_at=ticket.expires_at)
    ...
    def _execute_step(self,execution_id,index,tool,params,results):
        self.events.emit('state',state='acting',tool=tool.name,execution_id=execution_id);start=time.perf_counter()
        try:
            result=tool.handler(params);self._observe(f'tool.{tool.name}.ms',start);results[f'step{index+1}']={'ok':True,'result':result};self.memory.audit('tool','execute',{'execution_id':execution_id,'tool':tool.name,'params':params,'parameter_hash':parameter_hash(params),'ok':True})
        except Exception as exc:
            self._observe(f'tool.{tool.name}.ms',start)
            if self.telemetry:self.telemetry.increment(f'tool.{tool.name}.errors')
    ...
    def approve(self,approval_id:str):
        ...
        index=paused['index'];step=paused['plan']['steps'][index];tool=self.tools.get(step['tool']);params=step.get('parameters',{});self.approvals.consume(approval_id,paused['execution_id'],tool.name,params)
        with self._lock:self._paused.pop(approval_id,None)
        self.memory.audit('approval','approved',{'approval_id':approval_id,'execution_id':paused['execution_id'],'tool':tool.name,'parameter_hash':parameter_hash(params)});self.events.emit('approval.approved',approval_id=approval_id,execution_id=paused['execution_id'],tool=tool.name);results=paused['results'];self._execute_step(paused['execution_id'],index,tool,params,results);return self._continue(paused['execution_id'],paused['text'],paused['plan'],index+1,results,paused['history'])
    ...
    def _finalize(self,text,results,history):
        ...
        self._observe('model.chat_ms',start);self.memory.add_message('assistant',answer)
        if self.second_brain:
