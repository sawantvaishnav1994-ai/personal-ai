...
def normalize_runtime_state(value: RuntimeState | str | None) -> RuntimeState:
    ...
    try:
        ...
        return LEGACY_STATE_ALIASES.get(key, RuntimeState.IDLE)
...
LEGAL_TRANSITIONS = {
    ...
    RuntimeState.UNDERSTANDING: {
        RuntimeState.MEMORY_RETRIEVAL, RuntimeState.KNOWLEDGE_RETRIEVAL, RuntimeState.THINKING,
        RuntimeState.RESPONDING, RuntimeState.ERROR,
    },
    RuntimeState.MEMORY_RETRIEVAL: {RuntimeState.KNOWLEDGE_RETRIEVAL, RuntimeState.THINKING, RuntimeState.RESPONDING, RuntimeState.ERROR},
    RuntimeState.KNOWLEDGE_RETRIEVAL: {RuntimeState.MEMORY_RETRIEVAL, RuntimeState.THINKING, RuntimeState.RESPONDING, RuntimeState.ERROR},
    RuntimeState.THINKING: {RuntimeState.NEEDS_APPROVAL, RuntimeState.TOOL_ACTION, RuntimeState.RESPONDING, RuntimeState.BACKGROUND, RuntimeState.ERROR},
...
class RuntimeStateAuthority:
    """Canonical V1 runtime-state projection over existing runtime events.

    This object owns no execution authority. Existing security, P6/W7, model,
    Memory, Knowledge and P10 components remain authoritative. It normalizes
    their factual runtime events into one user-facing state contract.
    """
    ...
    def snapshot(self) -> StateSnapshot:
        with self._lock:
            return StateSnapshot(self._state, self._sequence, 'current')

    def transition(self, target: RuntimeState | str, *, reason: str, force: bool = False, **context) -> StateSnapshot:
        normalized = normalize_runtime_state(target)
        with self._lock:
            current = self._state
            if normalized == current:
                return StateSnapshot(current, self._sequence, reason)
            if not force and normalized not in LEGAL_TRANSITIONS[current]:
        ...
        self.events.emit('runtime.state', **payload)
        return StateSnapshot(normalized, sequence, reason)

    def _compat(self, event):
        target = normalize_runtime_state(event.get('state'))
        # Legacy state emitters reflect real subsystem work. During migration,
        # normalize them at this explicit boundary rather than letting UIs
        # interpret ad-hoc strings independently. Illegal stale transitions are
        # ignored instead of corrupting the canonical state machine.
        try:
            self.transition(target, reason=f"legacy:{event.get('event', 'state')}")
        except ValueError:
            return

    def _safe_transition(self, target: RuntimeState, reason: str, event: dict):
        try:
    ...
    def _bind_compatibility_events(self):
        ...
        mapping = {
            'turn.started': (RuntimeState.UNDERSTANDING, 'turn_started'),
            'turn.needs_approval': (RuntimeState.NEEDS_APPROVAL, 'approval_required'),
            'turn.completed': (RuntimeState.RESPONDING, 'turn_completed'),
            'turn.cancelled': (RuntimeState.IDLE, 'owner_cancelled'),
            'turn.failed': (RuntimeState.ERROR, 'turn_failed'),
            'approval.required': (RuntimeState.NEEDS_APPROVAL, 'approval_required'),
            'approval.approved': (RuntimeState.TOOL_ACTION, 'approval_approved'),
            'tool.unverified': (RuntimeState.WARNING, 'verification_warning'),
            'automation.started': (RuntimeState.BACKGROUND, 'background_started'),
            'workflow.started': (RuntimeState.BACKGROUND, 'background_started'),
        }
        for name, (target, reason) in mapping.items():
            self._subscriptions.append(
                self.events.subscribe(name, lambda event, t=target, r=reason: self._safe_transition(t, r, event))
            )
