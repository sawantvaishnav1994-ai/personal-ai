...
class P10TurnSelector:
    ...
    MAX_TEXT = 8000
    MULTI_STEP = re.compile(r'\b(first|then|after that|next|finally|step by step|multiple steps|end[- ]to[- ]end)\b', re.I)
    ...
    ACTION_VERBS = re.compile(r'\b(send|create|update|delete|book|schedule|publish|deploy|move|copy|email|message|upload|download|research|compare)\b', re.I)
    ...
    def select(self, text: str, *, explicit_background: bool = False, requested_actions: int = 0) -> TurnSelection:
        ...
        action_count = len(self.ACTION_VERBS.findall(value))
        if action_count >= 2: signals.append('multiple_action_verbs')

        strong = {'background_or_long_running', 'coordination_or_dependencies', 'multiple_governed_actions'}
    ...
    def apply_advisory(self, selection: TurnSelection, *, model_suggests_p10: bool | None) -> TurnSelection:
        """Model advice can reduce uncertainty but cannot grant P10 authority.

        A model-only positive classification never changes False -> True. A
        negative advisory may be recorded but deterministic owner/system policy
        ...
        return TurnSelection(
            selection.use_p10,
            selection.reason,
            selection.signals + (f'model_advisory:{str(bool(model_suggests_p10)).lower()}',),
            advisory_only=True,
        )
