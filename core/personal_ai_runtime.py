...
class CanonicalTurnRuntime:
    ...
    def __init__(self, executor, continuity, path: Path, *, events=None):
        self._executor=executor; self.continuity=continuity; self.events=events; self.path=Path(path); self.path.parent.mkdir(parents=True,exist_ok=True); self.lock=RLock(); self.p10_selector=P10TurnSelector(); self.autonomy=None
        with self._con() as con:
            ...
            cols={r[1] for r in con.execute('PRAGMA table_info(canonical_turns)')}
    ...
    def _resolve_conversation(self,*,device_id,conversation_id,surface):
        ...
        bundle=self.continuity.resume(str(device_id or f'local:{surface or "runtime"}'),event_limit=self.HISTORY_LIMIT*2);resolved=str((bundle.get('thread') or {}).get('id') or '')
    ...
    def _history(self,conversation_id,explicit_history):
        if explicit_history is not None:return [{'role':str(i.get('role')),'content':str(i.get('content'))} for i in list(explicit_history)[-self.HISTORY_LIMIT:] if isinstance(i,dict) and i.get('role') in {'user','assistant'} and str(i.get('content') or '').strip()]
    ...
    def _insert_started(self,**v):
        stamp=time.time()
        with self.lock,self._con() as con:con.execute("INSERT INTO canonical_turns(request_id,owner_id,conversation_id,device_id,session_id,surface,input_modality,privacy_level,risk_level,user_text,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,'started',?,?)",(v['request_id'],v['owner_id'],v['conversation_id'] or None,v['device_id'],v['session_id'],v['surface'],v['input_modality'],v['privacy_level'],v['risk_level'],v['user_text'],stamp,stamp))
    def _update(self,request_id,status,*,assistant_text=None,approval_id=None,error_code=None,p10_goal_id=None,p10_plan_id=None):
        with self.lock,self._con() as con:con.execute('UPDATE canonical_turns SET status=?,assistant_text=COALESCE(?,assistant_text),approval_id=COALESCE(?,approval_id),error_code=?,p10_goal_id=COALESCE(?,p10_goal_id),p10_plan_id=COALESCE(?,p10_plan_id),updated_at=? WHERE request_id=?',(status,assistant_text,approval_id,error_code,p10_goal_id,p10_plan_id,time.time(),request_id))
    def _validate_replay(self,existing,*,text,owner_id,device_id,session_id,conversation_id):
        if existing['owner_id']!=owner_id or existing['user_text']!=text:raise PermissionError('request_id is already bound to different turn content or authority')
        if (existing.get('device_id') or None)!=(device_id or None):raise PermissionError('request_id is already bound to a different device')
        if session_id and existing.get('session_id') and existing['session_id']!=session_id:raise PermissionError('request_id is already bound to a different trusted session')
        if conversation_id and existing.get('conversation_id') and existing['conversation_id']!=conversation_id:raise PermissionError('request_id is already bound to a different conversation')
        if existing['status']=='completed':
            answer=str(existing.get('assistant_text') or '');self._append(str(existing.get('conversation_id') or ''),device_id=existing.get('device_id'),kind='assistant_message',text=answer,event_id=f"{existing['request_id']}:assistant");self._emit('turn.replayed',request_id=existing['request_id'],status='completed');return answer
        raise TurnReplayBlocked(f"request {existing['request_id']} is {existing['status']}; resume its governed lifecycle instead of duplicating work")
    ...
    def _orchestrate(self,text,*,request_id,owner_id,device_id,session_id,privacy_level,risk_level,kwargs):
        selection=self.p10_selector.select(text,explicit_background=bool(kwargs.get('background',False)),requested_actions=int(kwargs.pop('requested_actions',0) or 0));self._emit('turn.orchestration_selected',request_id=request_id,use_p10=selection.use_p10,reason=selection.reason,signals=list(selection.signals))
        ...
        goal=self.autonomy.create_goal(text,owner_id=owner_id,desired_outcome='Complete the owner request through governed operations and return the verified outcome to this turn.',request_id=request_id,session_id=session_id,privacy=privacy_level,risk=risk_level);self._update(request_id,'planning',p10_goal_id=goal['id'])
        plan=self.autonomy.propose_plan_with_model(goal['id'],owner_id=owner_id,device_trusted=bool(device_id),session_fresh=bool(session_id));self._update(request_id,'orchestrating',p10_goal_id=goal['id'],p10_plan_id=plan['id']);self._emit('turn.p10_bound',request_id=request_id,goal_id=goal['id'],plan_id=plan['id'])
        while True:
    ...
    def chat(self,text,**kwargs):
        ...
        conversation_id=self._resolve_conversation(device_id=device_id,conversation_id=requested_conversation,surface=surface);history=self._history(conversation_id,kwargs.pop('conversation_history',None));kwargs['conversation_id']=conversation_id or None
        if history is not None:kwargs['conversation_history']=history
        kwargs['owner_id']=owner_id;self._append(conversation_id,device_id=device_id,kind='user_message',text=user_text,event_id=f'{request_id}:user');self._insert_started(request_id=request_id,owner_id=owner_id,conversation_id=conversation_id,device_id=device_id,session_id=session_id,surface=surface,input_modality=input_modality,privacy_level=privacy_level,risk_level=risk_level,user_text=user_text);self._emit('turn.started',request_id=request_id,conversation_id=conversation_id,device_id=device_id,surface=surface,input_modality=input_modality)
        ...
        except ConfirmationRequired as exc:self._update(request_id,'needs_approval',approval_id=exc.approval_id);self._emit('turn.needs_approval',request_id=request_id,conversation_id=conversation_id,approval_id=exc.approval_id);raise
        except ReauthenticationRequired:self._update(request_id,'needs_reauthentication',error_code='reauthentication_required');self._emit('turn.needs_reauthentication',request_id=request_id,conversation_id=conversation_id);raise
        except ExecutionCancelled:self._update(request_id,'cancelled',error_code='cancelled');self._emit('turn.cancelled',request_id=request_id,conversation_id=conversation_id);raise
        except TurnReplayBlocked:raise
        except Exception as exc:self._update(request_id,'failed',error_code=type(exc).__name__);self._emit('turn.failed',request_id=request_id,conversation_id=conversation_id,error_type=type(exc).__name__);raise
        finally:self._reset_context(tokens)
        answer=str(answer);self._update(request_id,'completed',assistant_text=answer);self._append(conversation_id,device_id=device_id,kind='assistant_message',text=answer,event_id=f'{request_id}:assistant');self._emit('turn.completed',request_id=request_id,conversation_id=conversation_id);return answer
    ...
    def _approval_call(self,approval_id,callback,**kwargs):
        ...
        answer=str(reply);self._update(turn['request_id'],'completed',assistant_text=answer);self._append(str(turn.get('conversation_id') or ''),device_id=turn.get('device_id'),kind='assistant_message',text=answer,event_id=f"{turn['request_id']}:assistant");self._emit('turn.completed',request_id=turn['request_id'],conversation_id=turn.get('conversation_id'));return reply
