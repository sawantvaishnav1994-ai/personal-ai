...
class CrossOperatorOrchestrator:
    ...
    def execute_step(self,operator,action,*,operation_class:str,target:str='',destination:str='',approved:bool=False,reauthenticated:bool=False,precondition:dict|None=None,expected_postcondition:dict|None=None,evidence_refs:list[str]|None=None):
        ...
        try:
            ...
            try:
                result=operator.execute(action,approved=approved,reauthenticated=reauthenticated)
            except TypeError:
            ...
            record=VerificationRecord(
                transaction_id=txid,action_id=action_id,dispatch_id=dispatch['dispatch_id'],idempotency_key=idempotency,
                operation_class=operation_class,target=target,destination=destination,
                precondition=precondition or {},expected_postcondition=expected_postcondition or {},observed_postcondition=observed,
                verifier_identity='w76.cross_operator',verifier_version='1',evidence_references=refs,
                evidence_checksum=_digest(material),verification_timestamp=time.time(),verification_fresh_until=time.time()+120,
                result=outcome,explanation=self._explanation(outcome),confidence=1.0 if outcome in {'verified_success','verified_no_effect','verified_failure'} else None,
            )
    ...
    def _refs(payload:dict)->list[str]:
        ...
        for key in ('evidence_ref','file_path','download_name'):
    ...
    def _explanation(result:str)->str:
        return {
            'verified_success':'The expected postcondition was independently observed.',
            'verified_no_effect':'Verification established that the attempted action produced no side effect.',
            'verified_partial':'Only part of the expected postcondition was observed. Recovery review is required.',
            'verified_failure':'Verification established that the expected postcondition was not achieved.',
            'unknown_outcome':'The action may have been dispatched, but evidence cannot establish the real-world outcome.',
            'cancelled_before_dispatch':'The action was cancelled before dispatch.',
            'blocked_before_dispatch':'Governance blocked the action before dispatch.',
            'recovery_review_required':'Owner recovery review is required before any further side effect.',
        }[result]
    ...
    def browser_download_recipe(self,*,origin:str,filename:str,mime:str,checksum:str,approved_folder:str)->dict:
        return {'steps':['validate_origin','validate_filename_mime','download_via_w7.4','checksum_file','store_in_approved_folder','verify_completion'],'origin':origin,'filename':filename,'mime':mime,'checksum':checksum,'approved_folder':approved_folder}

    def file_application_recipe(self,*,file_path:str,application_identity:str)->dict:
        return {'steps':['validate_file_policy','open_via_w7.5','verify_window','perform_bounded_action','save_output','verify_output_checksum_or_version'],'file_path':file_path,'application_identity':application_identity}

    def knowledge_upload_recipe(self,*,file_path:str,origin:str)->dict:
        return {'steps':['inspect_through_knowledge','create_derived_output','verify_local_output','request_trusted_action_approval','upload_via_w7.4','verify_uploaded_resource'],'file_path':file_path,'origin':origin}

    def extraction_report_recipe(self,*,origin:str,report_path:str)->dict:
        return {'steps':['extract_via_w7.4','sanitize_untrusted_content','create_local_report_via_w7.5','verify_report','stop_before_external_share_without_approval'],'origin':origin,'report_path':report_path}
