...
def build_runtime():
    events=EventBus(); memory=MemoryStore(settings.data_dir/'assistant.sqlite3'); models=ModelRouter(settings); vector=VectorStore(settings.data_dir/'vectors.sqlite3',models.embed); second_brain=SecondBrain(memory,models,vector); device_registry=DeviceRegistry(settings.data_dir/'devices.sqlite3'); device_gateway=DeviceGateway(device_registry,events); integrations,adapters=build_integrations(settings); tools=ToolRegistry(settings); executor=AgentExecutor(models=models,tools=tools,memory=memory,events=events,second_brain=second_brain); context_provider=lambda:{'devices':device_registry.list(),'integrations':integrations.list(),'memory_count':len(second_brain.graph().get('nodes',[]))}; automations=AutomationEngine(settings.data_dir/'automations.sqlite3',executor=executor,events=events,context_provider=context_provider); register_builtin_tools(tools,memory,settings,models=models,automation_engine=automations); vault=SecretVault(settings.data_dir/'vault.json',settings.vault_password) if settings.vault_password else None; voice=RealtimeVoiceSession(models,executor,events); return {'events':events,'memory':memory,'models':models,'second_brain':second_brain,'vector_store':vector,'device_registry':device_registry,'device_gateway':device_gateway,'tools':tools,'executor':executor,'automations':automations,'integrations':integrations,'integration_adapters':adapters,'vault':vault,'voice':voice}
def start_server(rt):
...
def main():
    ...
    win=MainWindow(events=rt['events'],executor=rt['executor'],memory=rt['memory'],runtime=rt); win.show(); code=app.exec(); rt['voice'].stop(); rt['automations'].stop(); return code
