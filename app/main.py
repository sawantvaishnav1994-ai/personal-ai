...
def build_runtime():
    ...
    if primary_thread is None:
        ...
        primary_thread_id = primary_thread['id']; continuity.set_active('desktop', primary_thread_id)
    ...
    plugins = PluginManifestRegistry(settings.data_dir / 'plugins'); plugins.load()
    ...
    executor = AgentExecutor(models=models, tools=tools, memory=memory, events=events, second_brain=second_brain, knowledge=knowledge, telemetry=telemetry)
    ...
    scenarios=CompetitiveScenarioSuite(runtime,benchmark);runtime['benchmark']=benchmark;runtime['model_evaluation']=model_evaluation;runtime['capability_scenarios']=scenarios;benchmark_tools.register(tools,benchmark,scenarios)
...
def main():
    ...
    window=MainWindow(events=runtime['events'],executor=runtime['executor'],memory=runtime['memory'],runtime=runtime);window.show()
    if runtime['preferences'].get('launch_voice_on_start'):window.toggle_voice()
    code=app.exec();runtime['voice'].stop();runtime['automations'].stop();runtime['telemetry'].persist();runtime['apns'].close();return code
