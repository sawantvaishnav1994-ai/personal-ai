...
def build_runtime():
    ...
    vault=SecretVault(settings.data_dir/'vault.json',settings.vault_password or None); integrations,adapters,oauth,oauth_providers=build_integrations(settings,vault); plugins=PluginManifestRegistry(settings.data_dir/'plugins');plugins.load();apns=APNsProvider(settings,device_registry,events)
    tools=ToolRegistry(settings);tools.set_autonomy_mode(str(preferences.get('autonomy_mode',settings.autonomy_mode)))
    ...
    voice=RealtimeVoiceSession(models,executor,events);voice_qualification=VoiceQualificationRecorder(settings.data_dir/'voice-qualification.sqlite3',events=events);p3_qualification=P3QualificationProgram(settings.data_dir/'p3-qualification.sqlite3');wake_phrase=WakePhraseGate(events,phrases=(str(preferences.get('wake_phrase','Hey Personal')),))
    events.subscribe('voice.transcript',lambda event:wake_phrase.accept(event.get('text','')));events.subscribe('state',lambda event:telemetry.increment(f"state.{event.get('state','unknown')}"));events.subscribe('voice.reply',lambda event:telemetry.increment('voice.replies'))
    ...
    # CanonicalTurnRuntime is constructed before FutureIntelligenceProgram to avoid
    # circular authority. Attach the already-qualified P10 orchestrator afterward;
    # no second runtime or executor is created.
    ...
    events.subscribe('conversation.user',lambda event:append_continuity('user_message',event.get('text'),event.get('device_id'),event.get('conversation_id')));events.subscribe('conversation.assistant',lambda event:append_continuity('assistant_message',event.get('text'),event.get('device_id'),event.get('conversation_id')))
...
def main():
    ...
    window=MainWindow(events=runtime['events'],executor=runtime['executor'],memory=runtime['memory'],runtime=runtime);window.show();floating_presence=FloatingPresence(runtime=runtime);runtime['floating_presence']=floating_presence;floating_presence.show()
    if runtime['preferences'].get('launch_voice_on_start'):window.toggle_voice()
    code=app.exec();floating_presence.close();runtime['voice'].stop();runtime['automations'].stop();runtime['telemetry'].persist();runtime['apns'].close();return code
