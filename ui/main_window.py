...
class MainWindow(QMainWindow):
    ...
    def __init__(self,*,events,executor,memory,runtime=None):
        ...
        self.setStyleSheet('''QMainWindow,QWidget{background:#040506;color:#eef4f8;font-family:Inter,Segoe UI,Arial} QLabel#brand{color:#c9d8e1;font-size:15px;font-weight:700;letter-spacing:3px} QLabel#muted{color:#72838e} QLabel#heroState{font-size:21px;font-weight:650;color:#edf7fb} QLineEdit{background:#090d11;border:1px solid #1c2a34;border-radius:18px;padding:13px 17px;font-size:15px;selection-background-color:#244656} QPushButton{background:transparent;border:1px solid transparent;border-radius:12px;padding:9px 13px;color:#9aabb5} QPushButton:hover{background:#0b1116;color:#eef7fb;border-color:#16242d} QPushButton:checked{background:#0d151b;color:#eef7fb;border-color:#233641} QPushButton#primary{background:#101c24;border:1px solid #29404d;color:#eaf7fb} QTextBrowser{background:#06090c;border:1px solid #131f27;border-radius:16px;padding:14px} QFrame#card{background:#070b0f;border:1px solid #14212a;border-radius:16px}''')
    ...
    def _dashboard_content(self,lay):
        graph=self.memory.graph();grid=QGridLayout();grid.addWidget(self._card('Second Brain',len(graph.get('nodes',[])),'memory nodes'),0,0);grid.addWidget(self._card('Devices',self._device_count(),'registered identities'),0,1);grid.addWidget(self._card('Voice','Ready' if self.runtime.get('voice') else 'Unavailable','Realtime + fallback runtime'),1,0);grid.addWidget(self._card('Security','Enforced','permissions · one-use approvals · vault · audit · device trust'),1,1);lay.addLayout(grid);b=QPushButton('Open Detailed Control Dashboard');b.setObjectName('primary');b.clicked.connect(self.open_control);lay.addWidget(b)
    def _more_content(self,lay):
        text=QTextBrowser();text.setHtml('<h3>Capabilities</h3><p>Voice-first interaction · Hey Personal wake phrase · browser and desktop control · Word/Excel/PowerPoint/PDF productivity · automations · account integrations · companion devices · secure notifications · Second Brain memory.</p><h3>Safety boundary</h3><p>External side effects require one-use execution-scoped approval. Device identity is revocable. Secrets stay in the vault or OS credential store.</p><h3>Extension model</h3><p>Use permission-scoped integrations and declarative plugin manifests. Arbitrary in-process Python plugins are intentionally not trusted by default.</p>');text.setMaximumHeight(330);lay.addWidget(text)
