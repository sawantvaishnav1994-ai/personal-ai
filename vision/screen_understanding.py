...
class ScreenUnderstanding:
    def __init__(self,models=None,data_dir:Path|None=None):
        self.models=models
    ...
    def capture(self,monitor:int=1)->Path:
        ...
        with mss.mss() as sct:
            mon=sct.monitors[monitor]; raw=sct.grab(mon)
            Image.frombytes("RGB",raw.size,raw.rgb).save(path)
        return path

    def prepare_payload(self,question:str="Describe the visible screen."):
        path=self.capture(); encoded=base64.b64encode(path.read_bytes()).decode("ascii")
        return {"screenshot":str(path),"question":question,
